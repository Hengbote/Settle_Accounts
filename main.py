...
class ProductEntryApp:
    ...
    def __init__(self, root):
        ...
        # ===== 启动流程 =====
        self._setup_window()        # 基础窗口设置（标题、大小、去原生标题栏、关闭事件绑定）
        self._build_title_bar()     # 先建标题栏
        self._build_content_area()  # 构建完 UI 框架后再连接数据库
        self.setup_databases()      # 设置数据库连接和表结构
        self.load_stores()          # 从主数据库加载店铺信息到内存
        self.sync_saved_stores()    # 启动时同步一次店铺数据，确保最新的店铺信息被加载到内存
    ...
    def sync_saved_stores(self):
        """把主数据库的店铺同步到 saved_data.db，保证外键目标存在

        注意：
        1. 这里只做插入/更新，不做删除
        2. 不能先 DELETE FROM stores，再全量重插
           因为 saved_data.db 里历史 entries 可能已经引用这些店铺，
           删除会触发外键问题
        """
        if not self.cursor_main or not self.cursor_saved:
            return
